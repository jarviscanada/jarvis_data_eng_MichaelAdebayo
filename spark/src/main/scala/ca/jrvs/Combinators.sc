...
val first5Elements = ls.take(5)
println(s"First 5 elements: $first5Elements")

val last5Elements = ls.takeRight(5)
println(s"Last 5 elements: $last5Elements")
...
val doubledFlatMap = numList.flatMap(lst => lst.map(_ * 2))
println(s"Using flatMap: $doubledFlatMap")

// Using map and flatten to achieve the same result
val doubledMapFlatten = numList.map(lst => lst.map(_ * 2)).flatten
println(s"Using map().flatten: $doubledMapFlatten")
